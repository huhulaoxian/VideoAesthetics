...
def uclidian(hist_a, hist_b):

    hist_a = np.array(hist_a)
    hist_b = np.array(hist_b)

    dit = 1 / (1 + dist(hist_a,hist_b))
...
def estimate_k(features):
    ...
    for i in range(len(features) - 1):
        if uclidian(features[i].hist, features[i + 1].hist) > threshold:
            k += 1
    return k
...
def remove_similar_keyframes(keyframes):
    ...
    for i in range(len(keyframes) - 1):
        ...
        for j in range(i + 1, len(keyframes)):
            ...
            if uclidian(keyframes[i].hist, keyframes[j].hist) > 0.5:
